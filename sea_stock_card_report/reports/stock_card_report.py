...
class StockCardView(models.TransientModel):
    _name = 'sea.stock.card.view'
    _description = 'Sea Stock Card View'
    _order = 'date'
...
class StockCardReport(models.TransientModel):
    ...
    _description = 'Sea Stock Card Report'
    ...
    results = fields.Many2many(
        comodel_name='sea.stock.card.view',
    ...
    def _compute_results(self):
        ...
        if self.lot_id.id is not False:
            self._cr.execute("""
                SELECT move.date, move.product_id, move.qty_done,
                    move.product_uom_qty, move.reference,
                    move.location_id, move.location_dest_id,move.remarks,
            ...
            """, (
                ...
                tuple(self.product_ids.ids), self.lot_id.id,
                self.date_to))
        else:
            self._cr.execute("""
            SELECT move.date, move.product_id, move.qty_done,
        ...
        ReportLine = self.env['sea.stock.card.view']
    ...
    def print_report(self, report_type='qweb'):
        ...
        action = report_type == 'xlsx' and self.env.ref(
            'sea_stock_card_report.action_stock_card_report_xlsx') or \
            self.env.ref('sea_stock_card_report.action_stock_card_report_pdf')
    ...
    def _get_html(self):
        ...
        if report:
            ...
            result['html'] = self.env.ref(
                'sea_stock_card_report.report_stock_card_report_html').render(
                    rcontext)
        return result
