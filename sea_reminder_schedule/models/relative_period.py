...
def return_start_and_end(interval, ttype, inclusive, period_direction):
    ...
    period_delta = relativedelta(days=days,
                                 weeks=weeks,
                                 months=months,
                                 years=years)
    # 3
    ...
    period_delta_2 = relativedelta(days=days,
                                   weeks=weeks,
                                   months=months,
                                   years=years)
    # 4
...
class relative_period(models.Model):
    ...
    @api.multi
    @api.depends("field_id", "period_value", "period_type", "inclusive_this",
                 "period_direction")
        ...
        for period in self:
            ...
            if field and interval > 0 and interval_type and period_direction:
                ...
                if interval_type == 'weeks':
                    # 2
                    calculated_start = calculated_start - relativedelta(
                        days=calculated_start.weekday())
                    calculated_end = calculated_end - relativedelta(
                        days=calculated_end.weekday())
                elif interval_type == 'months':
                    # 3
                    calculated_start = return_the_first_month_or_year_date(
                        calculated_start)
                    calculated_end = return_the_first_month_or_year_date(
                        calculated_end)
                elif interval_type == 'years':
                    # 4
                    calculated_start = return_the_first_month_or_year_date(
                        calculated_start, year=True)
                    calculated_end = return_the_first_month_or_year_date(
                        calculated_end, year=True)
                ...
                # 5
                title = "{} - {}".format(
                    calculated_start, calculated_end - relativedelta(days=1))
    ...
    inclusive_this = fields.Boolean(string="Including current",
                                    help="""
    ...
    _sql_constraints = [
        ('period_value_check', 'check (period_value>0)',
         _('Interval should be positive!')),
    ]
