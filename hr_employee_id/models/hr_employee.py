...
class HrEmployee(models.Model):
    ...
    s_identification_id = fields.Char(
        string='SeaCode',
        copy=False
    )

    _sql_constraints = [
        ('s_identification_id_uniq', 'unique(s_identification_id)',
         'The Employee Number must be unique across the company(s).'),
    ]

    @api.model
    def _generate_s_identification_id(self):
        ...
        for retry in range(50):
            ...
            if self.search_count([('s_identification_id', '=', employee_id)]):
                continue
    ...
    def create(self, vals):
        if not vals.get('s_identification_id'):
            vals['s_identification_id'] = self._generate_s_identification_id()
