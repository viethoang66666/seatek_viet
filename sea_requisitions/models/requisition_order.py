...
class MaterialPurchaseRequisition(models.Model):
    ...
    def unlink(self):
        for rec in self:
            if rec.state not in ('draft', 'cancel', 'reject'):
                raise Warning(
                    _('You can not delete Requisition Order which is not in draft or cancelled or rejected state.'
                      ))
        return super(MaterialPurchaseRequisition, self).unlink()

    name = fields.Char(string='Number', index=True, readonly=1,)
    state = fields.Selection(
        [('draft', 'New'), ('dept_confirm', 'Waiting Department Approval'),
         ('ir_approve', 'Waiting IR Approved'), ('approve', 'Approved'),
         ('stock', 'Purchase Order Created'), ('receive', 'Received'),
    ...
    employee_id = fields.Many2one('hr.employee', string='Employee', default=lambda self: self.env['hr.employee'].search(
                    [('user_id', '=', self.env.uid)], limit=1), required=True, copy=True,)
    ...
    requisition_line_ids = fields.One2many('requisition.order.line', 'requisition_id', string='Requisition Orders Line', copy=True,)
    date_end = fields.Date(string='Requisition Deadline', readonly=True, help='Last date for the product to be needed', copy=True,)
    date_done = fields.Date(string='Date Done', readonly=True, help='Date of Completion of Requisition Order',)
    ...
    purchase_order_ids = fields.One2many('purchase.order', 'requisition_id', string='Purchase Ordes',)
    custom_picking_type_id = fields.Many2one('stock.picking.type', string='Picking Type', copy=False,)

    @api.model
    def create(self, vals):
        name = self.env['ir.sequence'].sudo().next_by_code('requisition.order.seq')
    ...
    def requisition_confirm(self):
        for rec in self:
            manager_mail_template = self.env.ref(
                'sea_requisitions.email_confirm_requistion_order'
            )
    ...
    def manager_approve(self):
        for rec in self:
            ...
            employee_mail_template = self.env.ref(
                'sea_requisitions.email_requisition_iruser_custom'
            )
            email_iruser_template = self.env.ref(
                'sea_requisitions.email_material_requisition')
        ...
        for rec in self:
            ...
            if any(line.requisition_type == 'internal' for line in rec.requisition_line_ids):
                if not rec.dest_location_id:
                    if not rec.employee_id.dest_location_id.id and not rec.employee_id.department_id.dest_location_id.id:
                ...
                picking_vals = {
                    ...
                    rec.location_id.id,
                    'location_dest_id':
                    rec.dest_location_id and rec.dest_location_id.id
                    ...
                    for partner in line.partner_id:
                        if partner not in po_dict:
                            po_vals = {
                                ...
                                partner.id,
                                'currency_id':
                                rec.env.user.company_id.currency_id.id,
                                'date_order':
                                fields.Date.today(),
                                'company_id':
